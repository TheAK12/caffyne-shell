...
class LauncherAppItem(Button):
    def __init__(self, app: DesktopApp, launcher):
        ...
        self.box = Box(
                ...
                spacing=10,
                children=[
                    Image(pixbuf=app.get_icon_pixbuf(size=28)),
                    Box(
                        orientation="v",
                        children=[
                            Label(
...
class LauncherApplet(Applet):
    def __init__(self, parent):
        ...
        self._content_box = Box(orientation="v", spacing=6)
        ...
        self._scrolled_window.add(self._content_box)
        ...
        self._entry = Entry(
            ...
            on_activate=lambda *_: self._content_box.children[0].launch(),
        )
        ...
        self.connect("realize", lambda *_: (self.window.connect("notify::visible", self._on_visibility_changed), self.window.connect("key-press-event", self._on_key_press)))
    ...
    def _render_apps(self, apps: list):
        for child in self._content_box.children:
            child.destroy()
        self._app_count.label = f"Apps · {len(apps)}"
        self._content_box.children = [LauncherAppItem(a, self.window) for a in apps]
